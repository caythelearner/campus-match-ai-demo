...
打开公网链接后，先点 `管理员后台`，再进入顶部 tab `证据索引`。这个页面把 7 个核心技术主题对应的后台证据位置列出来。展示时建议保留 tab 名和卡片标题，这样技术讲解和公网后台能直接对应。

| 技术主题 | 公网后台位置 | 建议展示区域 | 画面必须能证明什么 |
|---|---|---|---|
| 第 1 页 系统链路 | `管理员后台 -> 总览`，也可先看 `证据索引` 第 1 张卡 | 星图看板、项目运行态、访谈抽取入口、Neo4j 产品图规模 | 这是实际后台，不是静态截图；能看到用户数、三元组数、Neo4j 节点/关系数 |
...
| 第 4 页 产品图谱 | `管理员后台 -> Neo4j图谱` | `产品图可视化`，点击 U001 或推荐节点后查看节点详情 | 图谱覆盖用户、推荐、GraphRAG、搭子、热度、首约、安全、治理，不只是画像三元组 |
...
如果页面里找不到对应内容，先确认页面右上角版本标记包含 `2026-06-16-ppt-public-map`。如果不是这个版本，浏览器开无痕窗口或在链接后加 `?v=20260616`。
...
这一节是技术展开说明和追问准备，不建议逐字念。真正上台限时口播用第 3 节。
...
后台证据：`管理员后台 -> 总览`。建议展示“星图看板”和“项目运行态”，画面里要保留 `Neo4j 产品图 5209/14600` 和版本标记。
...
- 前端是不是静态图片：不是。GitHub Pages 是静态 demo，但加载的是生成后的 JSON/CSV trace；本地 `8023` API 才支持实时 `/api/chat-rag`。
...
后台证据：`管理员后台 -> 访谈抽取`。建议展示“4 轮访谈 -> 三元组样例”和“本体约束与图谱质量”，这里能直接看到三元组，不需要打开本地文件。
...
后台证据：`管理员后台 -> 技术证据`。建议展示“意图图谱样例”“BM25 + 向量 + 图约束”“FAISS ANN 索引对比”三个区域。
...
- 后台 Neo4j 产品图，展示可拖拽节点。
- 一条推荐路径：`U001 - LIKES -> 剧本杀 <- LIKES - U010`，以及 `VALUES -> 边界感/安全感/坦诚沟通`。
- 右侧放节点和关系统计。

后台证据：`管理员后台 -> Neo4j图谱`。先点击或拖动一个节点，再展示“产品图可视化”和下面的节点详情。
...
后台证据：`管理员后台 -> 技术证据`，展示“GNN 链接预测样例”和“GCN 风险节点分类”；如果需要展示分数如何进推荐，再补 `管理员后台 -> 匹配推荐` 的分数拆解。
...
后台证据：`管理员后台 -> API Trace` 展示 RAG trace 明细；也可以到 `管理员后台 -> 技术证据` 展示“完整 RAG 流水线”。
...
后台证据：`管理员后台 -> 搭子任务` 展示任务候选排序；`管理员后台 -> 关系安全` 展示热度、首约安全和信用治理。
...
后台指向：`管理员后台 -> 总览`，指用户数、画像三元组、Neo4j 产品图规模。
...
后台指向：`管理员后台 -> 访谈抽取`，展示“4 轮访谈 -> 三元组样例”和“本体约束与图谱质量”。
...
后台指向：`管理员后台 -> 技术证据`，展示“意图图谱样例”“BM25 + 向量 + 图约束”“FAISS ANN 索引对比”。
...
后台指向：`管理员后台 -> Neo4j图谱`，点击节点后展示“产品图可视化”和节点详情。
...
后台指向：`管理员后台 -> 搭子任务` 和 `关系安全`，展示候选排序、热度、安全和信用治理。
...
- 切到 `管理员后台`。
- 浏览器缩放 90% 或 100%。
- 先点 `证据索引` 看 7 个核心技术主题对应的公网证据位置，再按演示顺序切换 `访谈抽取`、`技术证据`、`Neo4j图谱`、`API Trace`。
...
- 进入管理员后台。
- 先停留在 `总览`，再点一下 `证据索引` 说明每个核心技术主题都能在公网找到对应证据。
...
- 指一下 `画像三元组 3365`。
- 如果切到 `证据索引`，指出核心技术主题的后台证据位置都集中在这里，不需要打开本地代码。
